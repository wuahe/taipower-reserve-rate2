...
這是一個 TypeScript 全棧小服務，用來每 10 分鐘抓取台電官方資料，記錄今天的供電餘裕率，並在公開網頁顯示整天曲線。
...
- Zeabur 部署：讀取 `PORT`，並優先使用 Zeabur PostgreSQL 分項變數；若沒有分項變數才使用 `DATABASE_URL`、`POSTGRES_URI` / `POSTGRES_CONNECTION_STRING`。
...
DATABASE_INIT_MAX_ATTEMPTS=3
DATABASE_INIT_RETRY_DELAY_MS=1000
```
